...
import cairosvg
import pickle
from personal_engine import engine, engine_move
import svgrasterize
...
class Game():
    ...
    async def resign(self) -> None:
        ...
        self.game_done = True
        self.game_node = FinalizeGame(self.board, self.game_node.game(), self.players, (True, self.board.turn), False)
        await self.say_pgn()
    ...
    async def make_engine_move(self) -> bool:
        engine_move = await self._engine_move()
        await self._say(str(self.board.san(engine_move)))
...
if True:
    token_hider = str('here goes the token (but the github is public so be careful)')
    TOKEN = token_hider
...
async def on_message(message):
    ...
    if message.content.startswith('\start game'):
        await message.channel.send('Game setup in normal position!')
        board = Game(message.channel, engine=None)
        await board.show()
    elif message.content.startswith('\play engine'):
        await message.channel.send('Game setup in normal position!')
        board = Game(message.channel, engine=False)
        await board.show()
    elif message.content.startswith('\\end game'):
        await message.channel.send('Game ended')
        board.game_done = True
        await board.show()
    elif message.content.startswith('\\toggle'):
